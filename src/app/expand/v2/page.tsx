...
export default async function V2({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  ...
  const { isAdmin, funcoes } = await getAcesso();
  ...
  const canSeeMatrix = isAdmin || funcoes.includes("pmo");
  const matrixTasks: MatrixTask[] = canSeeMatrix
    ? etapas.filter(e => eff(e) !== "done").map(e => {
        const s = eff(e);
        const resp = resolveResp(e.responsavel_atual ?? e.responsavel);
        const dateStr = e.data_prevista
        ...
        return {
          id: e.id,
          titulo: e.titulo,
          cliente: cliMap.get(e.cliente_id) ?? null,
          responsavel: resp,
          dateStr,
          isPast: !!(e.data_prevista && e.data_prevista < hojeISO),
          isMarco: e.marco,
          statusColor: ST[s].bdr,
          urgencia: urgencia(e),
          quickviewUrl: qs({ qv: e.id }),
        };
      })
    : [];
    ...
    <>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 14 }}>
        <div style={{ display: "flex", background: "var(--panel-2)", borderRadius: 10, padding: 3, gap: 2 }}>
          {([
            ["monay",     "✦ Hoje"],
            ...(canSeeMatrix ? [["matrix", "⬛ Eisenhower"] as [string, string]] : []),
            ["lista",     "📋 Lista"],
            ["board",     "🗂 Board"],
            ["gantt",     "📊 Gantt"],
            ["timeline",  "📅 Agenda"],
      ...
      ═══════════════════════════════════════════════════════════════ */}
      {view === "matrix" && !canSeeMatrix && (
        <div style={{ padding: "40px 24px", textAlign: "center", color: "var(--dim)", fontSize: 13 }}>
          Acesso restrito a Administradores e PMO.
        </div>
      )}
      {view === "matrix" && canSeeMatrix && (
        <EisenhowerMatrix tasks={matrixTasks} canEdit={canSeeMatrix} />
      )}
