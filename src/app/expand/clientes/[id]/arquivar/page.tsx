...
type Cli = { id: string; nome: string; whatsapp_grupo: string | null; drive_folder_url: string | null; status: string | null; logo_url: string | null };
...
export default async function ArquivarWizard({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ step?: string }> }) {
  ...
  const { data: cData } = await supabase.from("expand_clientes").select("id, nome, whatsapp_grupo, drive_folder_url, status, logo_url").eq("id", id).maybeSingle();
  if (!cData) notFound();
    ...
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "0 0 64px" }}>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 8 }}>
          {cli.logo_url ? (
            <img src={cli.logo_url} alt={cli.nome} style={{ width: 44, height: 44, borderRadius: 10, objectFit: "contain", background: "var(--panel-2)", padding: 4, flexShrink: 0 }} />
          ) : (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div className="hx-glass" style={{ borderRadius: 16, padding: 24 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
              ...
              <button onClick={() => window.print()} className="hx-btn hx-btn-ghost" style={{ padding: "8px 16px", fontSize: 12.5 }}>
                🖨 Imprimir / PDF
              </button>
            </div>
          ...
          {/* Resumo do feedback antes de confirmar */}
          {feedbackSalvo && (
            <div style={{ padding: "14px 18px", borderRadius: 12, background: "var(--panel-2)", border: "1px solid var(--line)" }}>
              <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em", color: "var(--dim)", marginBottom: 6 }}>Feedback que será salvo na memória do PMO e do CS</div>
              <div style={{ fontSize: 13, fontWeight: 700 }}>{feedbackSalvo.motivo}</div>
          ...
          <div style={{ padding: "20px 24px", borderRadius: 16, border: "1px solid color-mix(in srgb, var(--red) 35%, transparent)", background: "color-mix(in srgb, var(--red) 5%, transparent)" }}>
            ...
            <div style={{ fontSize: 12.5, color: "var(--mut)", lineHeight: 1.6, marginBottom: 18 }}>
              Ao confirmar: o status da conta muda para <strong>churned</strong>, o feedback é salvo na memória do PMO e do CS, e o acesso ao portal é encerrado. Esta ação não pode ser desfeita.
            </div>
            <form action={confirmarEncerramento}>
              <input type="hidden" name="id" value={id} />
              <input type="hidden" name="nomeCli" value={cli.nome} />
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <button type="submit" className="hx-btn" style={{
                  padding: "11px 26px", fontSize: 13, fontWeight: 800,
                  background: "var(--red)", color: "#fff", border: "none", borderRadius: 10, cursor: "pointer",
                }}>
                  Encerrar conta de {cli.nome}
                </button>
